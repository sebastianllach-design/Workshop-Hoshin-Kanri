...
async function main() {
  const required = ['index.html','facilitator.html','cloud-sync.js','facilitator.js','firebase-config.js','firestore.rules','firestore.indexes.json','firebase.json','SETUP-FIREBASE.md','ARQUITECTURA-MULTIEQUIPO-v57.md','CHANGELOG-v57.md','INSTRUCCIONES-PUBLICACION-v57.md','PRUEBAS-v57.md','README.md','VERSION.txt'];
  required.forEach(file => assert(fs.existsSync(path.join(root, file)), `Falta ${file}.`));

  assert(html.includes('<meta name="application-version" content="v57">'), 'Falta metadato v57.');
  assert(html.includes("const APP_VERSION='v57'"), 'Falta APP_VERSION v57.');
  assert(facilitatorHtml.includes('<meta name="application-version" content="v57">'), 'Falta metadato v57 en el panel.');
