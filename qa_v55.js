...
async function main() {
  const required = ['index.html','facilitator.html','cloud-sync.js','facilitator.js','firebase-config.js','firestore.rules','firestore.indexes.json','firebase.json','SETUP-FIREBASE.md','ARQUITECTURA-MULTIEQUIPO-v55.md','CHANGELOG-v55.md','INSTRUCCIONES-PUBLICACION-v55.md','PRUEBAS-v55.md','README.md','VERSION.txt'];
  required.forEach(file => assert(fs.existsSync(path.join(root, file)), `Falta ${file}.`));

  assert(html.includes('<meta name="application-version" content="v55">'), 'Falta metadato v55.');
  assert(html.includes("const APP_VERSION='v55'"), 'Falta APP_VERSION v55.');
  assert(facilitatorHtml.includes('<meta name="application-version" content="v55">'), 'Falta metadato v55 en el panel.');
