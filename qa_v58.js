...
async function main() {
  const required = ['index.html','facilitator.html','cloud-sync.js','facilitator.js','firebase-config.js','firestore.rules','firestore.indexes.json','firebase.json','SETUP-FIREBASE.md','ARQUITECTURA-MULTIEQUIPO-v58.md','CHANGELOG-v58.md','INSTRUCCIONES-PUBLICACION-v58.md','PRUEBAS-v58.md','README.md','VERSION.txt'];
  required.forEach(file => assert(fs.existsSync(path.join(root, file)), `Falta ${file}.`));

  assert(html.includes('<meta name="application-version" content="v58">'), 'Falta metadato v58.');
  assert(html.includes("const APP_VERSION='v58'"), 'Falta APP_VERSION v58.');
  assert(facilitatorHtml.includes('<meta name="application-version" content="v58">'), 'Falta metadato v58 en el panel.');
  ...
  assert(facilitatorJs.includes('if(!confirm(`¿Eliminar definitivamente'), 'Falta la confirmación única de eliminación.');
  assert(!facilitatorJs.includes('Para confirmar, escribí el código') && !facilitatorJs.includes('typed.trim().toUpperCase()!==code'), 'La eliminación todavía solicita escribir el código.');
  ...
  assert(facilitatorHtml.includes('value="preguntas">Hoja 2 · Equipo y consultas') && facilitatorHtml.includes('value="despliegue">Hoja 3 · Variables y síntesis'), 'El panel no vincula el cronómetro con las hojas.');
  assert(facilitatorJs.includes('sheetId:stage.id') && facilitatorJs.includes('cycleId:`${stage.id}-${now}`'), 'El cronómetro no publica la hoja o el ciclo de control.');
  assert(cloudSync.includes('applyStageControl') && cloudSync.includes('onStageActivated') && cloudSync.includes('onStageExpired'), 'Falta el control remoto de vistas y vencimientos.');
  assert(html.includes('function isSectionInteractionLocked') && html.includes("t.round1=snapshot();t.stageClosures.despliegue=control.token"), 'Faltan el bloqueo por hoja o la generación automática de la presentación.');
  assert(html.includes("guardStageInteraction('preguntas'") && html.includes("guardStageInteraction('despliegue'"), 'Las acciones de las Hojas 2 y 3 no están protegidas después del vencimiento.');
  assert(html.includes('<b>Visión de la empresa</b>') && html.includes('5 nuevas sucursales para 2029'), 'Falta la visión actualizada de la empresa.');
  assert(!html.includes('Dirección estratégica de largo plazo'), 'Continúa visible el nombre anterior del bloque de visión.');
  assert(html.indexOf('<b>Visión de la empresa</b>')>html.indexOf('01. Perfil de Deportes Andina') && html.indexOf('<b>Visión de la empresa</b>')<html.indexOf('02. Evolución de la compañía'), 'La visión no está al final del perfil de la empresa.');
  assert(html.includes('<h3>1. Equipo y roles</h3>') && html.includes('<h3>2. Consultas a Dirección</h3>'), 'Los dos bloques de la Hoja 2 no son desplegables.');
  assert(html.includes('.matrix-guide:not([open]) .fold-action::before{content:"Mostrar"}'), 'La guía de la Matriz no restaura la etiqueta Mostrar al cerrarse.');
  assert(html.includes("w.matches('#caso .case-matrix')"), 'El aviso de desplazamiento continúa dentro de la lectura estratégica.');
  assert(html.includes('<b>Índice de equilibrio</b><p>${snap.balance}/100') && !html.includes('<span class="report-score-balance">Índice de equilibrio'), 'El Índice de equilibrio no es un indicador independiente del reporte.');
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'Eliminación completa con una única confirmación',
      'Avance por hojas y bloqueo automático al vencer el tiempo',
      'Presentación de Hoja 3 congelada automáticamente',
      'Ajustes visuales solicitados en Hojas 1 a 4',
